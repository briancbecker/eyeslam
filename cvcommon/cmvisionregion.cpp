...
RegionProcessing::RegionProcessing()
{
	...
	colorlist = new CMVision::ColorRegionList(256);
}
...
void RegionProcessing::encodeRuns(cv::Mat &tmap, CMVision::RunList * runlist)
...
// only have to look at the points where values change.
{
  ...
  raw8 * map = tmap.data;
  int width=tmap.step;
  int height=tmap.rows;

  assert(tmap.type() == CV_8UC1);
...
void RegionProcessing::processThresholded(cv::Mat &_img_thresholded, int min_blob_area) 
{
...
void RegionProcessing::drawRegion(cv::Mat img, const Region &r, int color /*= 255*/)
{
	...
	int parent = runs[r.run_start].parent;
	for (int i = 0; i < runlist->getUsedRuns(); i++)
	{
		if (runs[i].parent == parent && runs[i].x >= r.x1 && runs[i].y >= r.y1 && runs[i].x + runs[i].width <= r.x2 && runs[i].y <= r.y2)
			memset(img.data + img.step*runs[i].y + runs[i].x, color, runs[i].width);
	}
}
